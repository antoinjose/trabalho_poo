...
<head>
    ...
    <title>Editar Ordem de Serviço - Sistema SOM</title>
        ...
        <div class="bg-white shadow-sm border-b border-gray-200">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold text-gray-900">Editar Ordem de Serviço</h1>
                        <p class="text-gray-500 mt-1">Atualize os dados desta ordem</p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white rounded-lg shadow p-6">
                <form action="{{ route('ordens.update', $ordem->id) }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <select id="cliente_id" name="cliente_id" class="w-full px-4 py-2 border @error('cliente_id') border-red-500 @else border-gray-300 @enderror rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
                            ...
                            @foreach($clientes as $cliente)
                                <option value="{{ $cliente->id }}" {{ old('cliente_id', $ordem->cliente_id) == $cliente->id ? 'selected' : '' }}>{{ $cliente->nome }}</option>
                            @endforeach
                        </select>
                        @error('cliente_id')
                    ...
                    <div>
                        ...
                        <textarea id="descricao" name="descricao" rows="5" class="w-full px-4 py-3 border @error('descricao') border-red-500 @else border-gray-300 @enderror rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Detalhe os serviços a realizar" required>{{ old('descricao', $ordem->descricao) }}</textarea>
                        @error('descricao')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        <div>
                            ...
                            <select id="status" name="status" class="w-full px-4 py-2 border @error('status') border-red-500 @else border-gray-300 @enderror rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
                                @foreach(['ABERTA', 'EM ANDAMENTO', 'CONCLUÍDA', 'CANCELADA'] as $status)
                                    <option value="{{ $status }}" {{ old('status', $ordem->status) === $status ? 'selected' : '' }}>{{ $status }}</option>
                                @endforeach
                            </select>
                            @error('status')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        ...
                        <div>
                            ...
                            <input type="number" step="0.01" min="0" id="valor" name="valor" value="{{ old('valor', $ordem->valor) }}" class="w-full px-4 py-2 border @error('valor') border-red-500 @else border-gray-300 @enderror rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="0.00">
                            @error('valor')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="date" id="data_entrega" name="data_entrega" value="{{ old('data_entrega', $ordem->data_entrega?->format('Y-m-d')) }}" class="w-full px-4 py-2 border @error('data_entrega') border-red-500 @else border-gray-300 @enderror rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            @error('data_entrega')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
